...
import { X, HandCoins, Wallet } from 'lucide-react'
...
export function AddCreditPaymentModal({ order, onClose, onDone }: Props) {
  ...
  const canSubmit = parsed > 0 && parsed <= balance && !addPayment.isPending

  function handleSubmit() {
    if (!canSubmit) return
    addPayment.mutate(
      { order_id: order.id, amount: parsed, method },
      { onSuccess: () => onDone() },
    )
  }
  ...
  return (
    ...
    >
      <div
        className="flex w-full max-w-md flex-col rounded-[14px] bg-white shadow-[0_20px_60px_rgba(0,0,0,0.3)]"
