...
export function AddPaymentModal({
  ...
  const willPayoff = parsed === balancePending && parsed > 0
  const canSubmit =
    parsed > 0 && parsed <= balancePending && visibleMethods.length > 0
  ...
  function handleSubmit() {
    ...
    if (willPayoff && completeOnPayoff) {
      completeLayaway.mutate(
        {
          id: layawayId,
          final_payment: { amount: parsed, method, notes: trimmedNotes },
        },
        {
          onSuccess: () => {
            onCompleted()
